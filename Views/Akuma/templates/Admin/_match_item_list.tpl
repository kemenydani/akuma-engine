	...
	<table class="table table-responsive table-condensed">
		...
		{if $table}
			...
			{if $table[$i]}
				...
				{if $item[$table[$i].key] != null}
				    
				    {if $table[$i].key == 'home_team_id'}
					{team team_id=$item[$table[$i].key]}
		...
		<td>
			...
			{if isset($item.active) && $item.active == 1}
			    <a  
			       ...
			       data-message="{$item['enemy_name']} vs. {team team_id=$item['home_team_id']} <br> ID: {$item['match_id']}" 
