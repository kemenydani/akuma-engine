    ...
    <div class="col-sm-11">
        <div class="input-group">
            ...
            <span class="input-group-btn">
                <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#{$name}_fileMgr">Browse image</button>
            </span>
            <input value="{if $value}{$value}{/if}" type="text" class="form-control" name="{$name}" id="field_{$name}" placeholder="Browse image">
  ...
  <div class="modal-dialog" style="width: 90%" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" style="margin: 0px;">
          <div class="row">
		<iframe  width="100%" height="700px" frameborder="0"src="{$base}Core/Assets/ResponsiveFileUploader/dialog.php?type=1&field_id=field_{$name}&relative_url=1"></iframe>

              <script>
                  function responsive_filemanager_callback(field_id){
                      var url=jQuery('#'+field_id).val();
                      $('#'+field_id).val(url);
                      $("#{$name}_fileMgr").modal('hide');
                  }
              </script>
